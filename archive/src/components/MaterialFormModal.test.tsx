...
describe('MaterialFormModal Dimension Input Integration', () => {
  ...
  it('should correctly parse and save dimension inputs with units', async () => {
    ...
    await userEvent.type(within(materialForm).getByLabelText('Nome do Material'), 'Material com Dimensões');
  ...
  await userEvent.click(within(materialForm).getByRole('button', { name: /adicionar componente/i }));
    ...
    const saveButton = screen.getByRole('button', { name: /salvar material/i });
    await act(async () => {
  ...
  it('should add and edit a component correctly', async () => {
    ...
    expect(within(materialForm).getByLabelText('Nome do Material')).toHaveValue('Material Existente');
