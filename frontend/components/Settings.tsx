...
export const Settings: React.FC = () => {
  ...
  const handleMigrateLocalStorage = () => {
    try {
      ...
      setMigrateStatus('Migration failed.');
  ...
  return (
    <div className="container mx-auto max-w-2xl">
      ...
      <div className="bg-surface dark:bg-slate-800 p-6 rounded-lg shadow-md space-y-6">
        ...
        <div>
          <label className="block text-sm font-medium text-textSecondary dark:text-slate-300 mb-1">Informações de Contato</label>
          <textarea 
            name="companyContact"
        ...
        <div>
            <label className="block text-sm font-medium text-textSecondary dark:text-slate-300 mb-1">Logotipo da Empresa</label>
            <div className="mt-2 flex items-center space-x-4">
                {localSettings.companyLogo && <img src={localSettings.companyLogo} alt="logo" className="h-16 w-16 object-contain border p-1 rounded-md bg-gray-200 dark:bg-slate-600 border-gray-600 dark:border-slate-500"/>}
                <input type="file" accept="image/*" onChange={handleLogoChange} className="block text-sm text-textSecondary dark:text-slate-300 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-900 file:text-blue-300 hover:file:bg-blue-800 transition-colors"/>
            </div>
        </div>
