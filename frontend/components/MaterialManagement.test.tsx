...
describe('MaterialManagement', () => {
  ...
  expect(screen.getByRole('textbox', { name: /Nome do Material/i })).toBeInTheDocument();
  ...
  expect(screen.getByRole('button', { name: /criar material/i })).toBeInTheDocument();
  });
  ...
  it('should add a new component field when "Adicionar Componente" button is clicked', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /Novo Material/i })); // Adicionado para abrir o modal
    await waitFor(() => expect(screen.getByRole('button', { name: /Adicionar Componente/i })).toBeInTheDocument());
  ...
  fireEvent.change(screen.getByRole('textbox', { name: /Nome do Material/i }), { target: { value: 'Material Principal' } });
